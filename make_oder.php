...
if (isset($_POST['make'])) {
  ...
  if (empty($_POST["order_code"]) || empty($_POST["customer_name"]) || empty($_GET['prod_price']) || empty($_POST['pay_method'])) {
    $err = "Blank Values Not Accepted";
  } else {
    // Retrieve form and URL values
    ...
    $order_status = "paid"; // Set default order status here
    $pay_method = $_POST['pay_method']; // New addition for pay method

    // Insert captured information into the database table
    $postQuery = "INSERT INTO rpos_orders (prod_qty, order_id, order_code, customer_id, customer_name, prod_id, prod_name, prod_price, order_status, pay_method) VALUES(?,?,?,?,?,?,?,?,?,?)";
    $postStmt = $mysqli->prepare($postQuery);

    // Bind parameters including pay_method
    $rc = $postStmt->bind_param('ssssssssss', $prod_qty, $order_id, $order_code, $customer_id, $customer_name, $prod_id, $prod_name, $prod_price, $order_status, $pay_method);
    $postStmt->execute();
                ...
                ?>
                  <div class="form-row">
                    <div class="col-md-6">
                      ...
                      <input type="text" readonly name="prod_price" value="R <?php echo $prod->prod_price; ?>" class="form-control">
                    </div>
                    <div class="col-md-6">
